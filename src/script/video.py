...
def main():
    ...
    while True:
        ...
        for r in results:
            ...
            for box in boxes:
                ...
                if cls_id == 0:
                    ...
                    # --- PREDICTABLE MOCK PPE VIOLATION FOR DEMO ---
                    # To make the demo look great, we will trigger a violation 
                    # if the person moves to the RIGHT side of the camera!
        ...
        # Every 15 frames (approx 0.5 seconds), send the payload to the backend
        if frame_count % 15 == 0:
            ...
            # Send the JSON payload over HTTP to the Vite dev server
            vite_port = os.environ.get('VITE_PORT', '5173')
            backend_url = f"http://localhost:{vite_port}/api/cv/frame"
            try:
        ...
        # Encode frame as JPEG and send to Vite proxy for MJPEG streaming
        _, buffer = cv2.imencode('.jpg', frame)
        frame_bytes = buffer.tobytes()
        try:
            vite_port = os.environ.get('VITE_PORT', '5173')
            requests.post(f"http://localhost:{vite_port}/api/cv/video_frame", data=frame_bytes, headers={'Content-Type': 'application/octet-stream'}, timeout=0.2)
        except requests.exceptions.RequestException as e:
            print(f"Failed to send video frame: {e}") # Ignore if Vite server is overwhelmed or restarting

        # Add a tiny delay to cap frame rate somewhat and free CPU
        time.sleep(0.03)
